...
export function Overview({onProjects,onOpenProject}:{onProjects:()=>void;onOpenProject:(id?:string)=>void}) {
...
export function Deployments({onOpen}:{onOpen:(id?:string)=>void}) {
...
export function Observability(){const [tab,setTab]=useState("Overview");return <><Header title="Observability" action={<button className="vc-btn" onClick={()=>toast.info("Refreshing provider telemetry")}>Refresh</button>}/><div className="vc-filterbar">{["Overview","Logs","Metrics","Errors","Requests","Uptime"].map(x=><button key={x} className={tab===x?"active":""} onClick={()=>setTab(x)}>{x}</button>)}</div><div className="vc-grid-3"><Stat label="Requests" value="—"/><Stat label="Errors" value="—"/><Stat label="Latency" value="—"/></div><div className="vc-card"><h3>{tab}</h3><Empty title="No runtime data" body="Connect a deployment/runtime to populate searchable telemetry and operational metrics."/></div></>;}
