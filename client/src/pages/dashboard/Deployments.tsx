...
function DeploymentEnvVars({ organizationId, projectId, environment }: { organizationId: string; projectId: string; environment: "production" | "preview" | "development" }) {
  ...
  const remove = trpc.deployments.envVars.remove.useMutation({ onSuccess: () => void query.refetch(), onError: error => toast.error(error.message) });
  return <div className="vc-card vc-settings-editor"><span className="vc-eyebrow">{environment} environment</span><h3>Environment variables</h3><p>Secret values are never returned to the browser. The current adapter stores only a masked value and provider reference.</p><div className="vc-form-grid"><label>Name<input value={name} onChange={event => setName(event.target.value.toUpperCase())} placeholder="DATABASE_URL" /></label><label>Provider reference<input value={valueRef} onChange={event => setValueRef(event.target.value)} placeholder="secret://provider/ref" /></label></div><button className="vc-btn primary" disabled={!name || !valueRef || save.isPending} onClick={() => save.mutate({ organizationId, projectId, environment, name, valueRef, isSecret: true })}>Save variable</button>{query.data?.map(variable => <div className="vc-list-row" key={variable.id}><span><strong>{variable.name}</strong><small>{variable.masked_value}</small></span><button className="vc-btn" onClick={() => remove.mutate({ id: variable.id })}>Remove</button></div>)}</div>;
}
