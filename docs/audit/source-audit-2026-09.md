...
| C10 | `projects.update` could change `slug`, which is the engine application's name. **Fixed**: the worker records the application in `projects.provider_resource_id` on the first deploy, and `projects.update` now refuses a slug change once that reference exists (the engine offers no rename the control plane may call), naming the reason; the Settings page disables the field and shows the same hint. Before the first deploy the slug still changes freely, and the display name is never locked. | `apps/api/src/procedures/organizations.ts:updateProject`; `apps/web/src/pages/pages.tsx:ProjectSettingsPage`; `apps/worker/src/deployment-executor.ts` | Implemented (refuses divergence) |
...
| S7 | **Fixed.** The edge **decided-traffic** view is wired: `security_events` (org RLS, migration `0010`) → `security.events.list` (`apps/api/src/procedures/index.ts:490`) → the Security page's "Edge decisions" table, which shows denied/challenged/allowed per request. The *incident lifecycle* is now wired too: `security_incidents` (org RLS + service-role observation, migration `0019`) → the detector opens an incident on a rejected policy distribution (`apps/worker/src/policy-job.ts`, and the synchronous `distributeSecurityPolicy`) → `security.incidents.list/transition` (`apps/api/src/procedures/security.ts`) → the Security page's "Incidents" table with triage/close. The API and the table's trigger enforce the same rules: triage before close, a resolution to close, no reopen. | `supabase/migrations/0019_security_incidents.sql`; `apps/api/src/procedures/security.ts`; `apps/worker/src/policy-job.ts`; `apps/web/src/pages/pages.tsx`; `tests/isolation/data-security-writes.test.ts` | Wired |
