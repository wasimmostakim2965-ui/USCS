...
export default function Auth({onBack,onSuccess,initialMode='signup',demoMode=false}:{onBack:()=>void;onSuccess:()=>void;initialMode?:'signup'|'signin';demoMode?:boolean}) {
  ...
  const persistDetails=async()=>{
    ...
    setBusy(true);setError('');
    if(!dob){setError('Enter your date of birth.');setBusy(false);return}
    if(!nationality){setError('Select your nationality.');setBusy(false);return}
    try{await saveProfile(userId,{account_type:'personal',country:residenceCountry,full_name:fullName.trim(),phone:phone.trim(),business_name:null,business_type:null,business_category:null,onboarding_status:'details_pending'});await saveKyc(userId,{legal_name:fullName.trim(),date_of_birth:dob,nationality});await recordAudit(userId,'onboarding.details_saved','profile');go(4)}catch(e){setError((e as Error).message||'We could not save your details.')}finally{setBusy(false)}
  };
  ...
  return <AuthShell onBack={onBack} title={step===2&&googleSignup?'Secure your Paywai account':STEP_TITLES[step]} kicker="OPEN A PAYWAI ACCOUNT" step={`${String(step).padStart(2,'0')} / 07`}>
    ...
    {step===3&&<Panel title="Personal details" text="Enter your legal personal information exactly as it appears on your official records. These details are used for account identification and compliance checks."><div className="form-grid"><Field label="Full legal name"><input value={fullName} onChange={e=>setFullName(e.target.value)} placeholder="First and last name" autoComplete="name"/></Field><Field label="Date of birth"><input type="date" value={dob} onChange={e=>setDob(e.target.value)} autoComplete="bday"/></Field><Field label="Nationality"><select value={nationality} onChange={e=>setNationality(e.target.value)}>{COUNTRIES.map(c=><option key={c}>{c}</option>)}</select></Field><Field label="Country of residence"><select value={residenceCountry} onChange={e=>setResidenceCountry(e.target.value)}>{COUNTRIES.map(c=><option key={c}>{c}</option>)}</select></Field></div><Field label="Mobile number"><input value={phone} onChange={e=>setPhone(e.target.value)} placeholder="+880 1XXX XXXXXX" autoComplete="tel"/></Field><div className="notice"><span>Use your real legal details. You may be asked to verify them against an official identity document.</span></div>{error&&<div className="error-note">{error}</div>}<Actions back={()=>go(1)} next={persistDetails} busy={busy} label="Continue"/></Panel>}
