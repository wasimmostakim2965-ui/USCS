...
export default function Auth({onBack,onSuccess,initialMode='signup',demoMode=false}:{onBack:()=>void;onSuccess:()=>void;initialMode?:'signup'|'signin';demoMode?:boolean}) {
  ...
  return <AuthShell onBack={onBack} title={step===2&&googleSignup?'Secure your Paywai account':STEP_TITLES[step]} kicker="OPEN A PAYWAI ACCOUNT" step={`${String(step).padStart(2,'0')} / 07`}>
    ...
    {step===4&&<Panel title="Identity document" text="Enter the details exactly as they appear on your passport or national ID."><Field label="Legal full name"><input value={legalName} onChange={e=>setLegalName(e.target.value)} placeholder="Exactly as shown on your ID"/></Field><div className="form-grid"><Field label="Date of birth"><input type="date" value={dob} onChange={e=>setDob(e.target.value)}/></Field><Field label="Nationality"><select value={nationality} onChange={e=>setNationality(e.target.value)}>{demoMode&&<option>TEST</option>}{COUNTRIES.map(c=><option key={c}>{c}</option>)}</select></Field><Field label="Document type"><select value={documentType} onChange={e=>setDocumentType(e.target.value)}>{demoMode&&<option>TEST</option>}<option>National ID</option><option>Passport</option><option>Driver's license</option></select></Field><Field label="Document country"><select value={documentCountry} onChange={e=>setDocumentCountry(e.target.value)}>{COUNTRIES.map(c=><option key={c}>{c}</option>)}</select></Field></div><Field label="Document number"><input value={documentNumber} onChange={e=>setDocumentNumber(e.target.value)} placeholder="Enter the number on the document"/></Field><Field label="Occupation"><input value={occupation} onChange={e=>setOccupation(e.target.value)} placeholder="Occupation or profession"/></Field><div className="notice"><span>Document authenticity and liveness checks can be performed by the identity provider before approval.</span></div>{error&&<div className="error-note">{error}</div>}<Actions back={()=>go(3)} next={persistIdentity} busy={busy} label="Save and continue"/></Panel>}
