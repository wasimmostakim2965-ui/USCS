...
export default function Auth({onBack,onSuccess,initialMode='signup',demoMode=false}:{onBack:()=>void;onSuccess:()=>void;initialMode?:'signup'|'signin';demoMode?:boolean}) {
  ...
  useEffect(()=>{void (async()=>{
    try{
      ...
      if(normalized.length && !normalized.some((x:GeoCountry)=>x.name===residenceCountry))setResidenceCountry(normalized.find((x:GeoCountry)=>x.code==='BD')?.name??normalized[0].name);
    }catch{
  ...
  const persistDetails=async()=>{
    ...
    if(!nationality||!countryOfBirth||!placeOfBirth.trim()){setError('Complete your birth and nationality details.');return}
    if(!gender||!maritalStatus){setError('Select your gender and marital status.');return}
    setBusy(true);setError('');
    try{const composed=[firstName.trim(),middleName.trim(),lastName.trim()].filter(Boolean).join(' ');setFullName(composed);await saveProfile(userId,{account_type:'personal',country:residenceCountry,full_name:composed,phone:`${dialCodes[residenceCountry]??''} ${phone.trim()}`.trim(),business_name:null,business_type:null,business_category:null,onboarding_status:'details_pending'});await saveKyc(userId,{legal_name:composed,first_name:firstName.trim(),middle_name:middleName.trim()||null,last_name:lastName.trim(),date_of_birth:dob,place_of_birth:placeOfBirth.trim(),country_of_birth:countryOfBirth,secondary_nationality:secondaryNationality||null,gender,marital_status,nationality,occupation:occupation.trim()||null,employer:employer.trim()||null});await recordAudit(userId,'onboarding.details_saved','profile');go(4)}catch(e){setError((e as Error).message||'We could not save your personal details.')}finally{setBusy(false)}
  };
  ...
  return <AuthShell onBack={onBack} title={step===2&&googleSignup?'Secure your Paywai account':STEP_TITLES[step]} kicker="OPEN A PAYWAI ACCOUNT" step={`${String(step).padStart(2,'0')} / 07`}>
    ...
    {step===5&&<Panel title="Residential address & tax residence" text="Provide your current residential address and tax residence."><Field label="Country of residence"><select value={residenceCountry} onChange={e=>{setResidenceCountry(e.target.value);setRegion('');setCity('')}}>{(geoCountries.length?geoCountries.map(c=>c.name):FALLBACK_COUNTRIES).map(c=><option key={c}>{c}</option>)}</select></Field><Field label="Street address"><input value={address} onChange={e=>setAddress(e.target.value)} placeholder="House, street and area"/></Field><div className="form-grid"><Field label="State / province / division"><select value={region} onChange={e=>{setRegion(e.target.value);setCity('')}} disabled={geoLoading}><option value="">{geoLoading?'Loading divisions…':states.length?'Select division':'No division data available'}</option>{states.map(s=><option key={s.name}>{s.name}</option>)}</select></Field><Field label="City"><select value={city} onChange={e=>setCity(e.target.value)} disabled={!region||cityLoading}><option value="">{cityLoading?'Loading cities…':region?(cities.length?'Select city':'No city data available'):'Select division first'}</option>{cities.map(name=><option key={name}>{name}</option>)}</select></Field><Field label="Postal code"><input value={postal} onChange={e=>setPostal(e.target.value)} placeholder="Postal code"/></Field><Field label="Tax residence country"><select value={taxResidence} onChange={e=>setTaxResidence(e.target.value)}>{(geoCountries.length?geoCountries.map(c=>c.name):FALLBACK_COUNTRIES).map(c=><option key={c}>{c}</option>)}</select></Field></div>{error&&<div className="error-note">{error}</div>}<Actions back={()=>go(4)} next={persistResidence} busy={busy} label="Continue"/></Panel>}
