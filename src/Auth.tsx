...
export default function Auth({onBack,onSuccess,initialMode='signup'}:{onBack:()=>void;onSuccess:()=>void;initialMode?:'signup'|'signin'}) {
  ...
  useEffect(()=>{void (async()=>{const {data}=await supabase.auth.getSession();if(data.session?.user){setUserId(data.session.user.id);setEmail(data.session.user.email??'');}})();},[]);
  ...
  const createAccount=async()=>{
    ...
    try{await sendSignupCode();go(2)}catch(err){setError((err as Error).message||'We could not send the verification code.');}
    setBusy(false);
  };
  ...
  const back=()=>{if(step===1)return onBack();if(step===2)return go(1);if(step===3)return go(1);if(step===4)return go(3);if(step===5)return go(4);if(step===6)return go(5);return go(1)};
...
function Panel({children,icon,title,text}:{children:React.ReactNode;icon:React.ReactNode;title:string;text:string}){return <div className="verification-panel"><div className="verification-icon">{icon}</div><b>{title}</b><p>{text}</p>{children}</div>}
